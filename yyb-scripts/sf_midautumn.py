...
用法：
  1. YYB 授权登录：自动遍历 YYB 所有账号取码登录（可用 YYB_REF 指定单账号）
     兼容旧方式：无 YYB 账号时回退 sfsyUrl 环境变量（多账号换行分隔，支持 URL 或 Cookie 格式）
  2. 可选环境变量：
     SFBF          并发数（默认 1）
...
YYB_REF = os.getenv("YYB_REF", "")                       # 指定账号 ref，空=遍历所有账号
SF_WX_APPID = os.getenv("SF_WX_APPID", "wxd4185d00bf7e08ac")   # 顺丰速运小程序 appid
SF_PUBLIC_ID = os.getenv("SF_PUBLIC_ID", "gh_f9d9fca26a50")    # 小程序原始 ID
...
def yyb_get_accounts():
    try:
        ...
        print(f"❌ 获取 YYB 账号列表失败: {str(e)[:80]}")
    return []


def yyb_get_code(ref):
    """YYB 取小程序一次性 wx.login code"""
    try:
        resp = requests.post(f"{YYB_BASE_URL}/wxapp/getCode", timeout=60,
                             json={"ref": str(ref), "app_id": SF_WX_APPID})
        data = resp.json()
        if data.get("code") == 0:
            ...
            return None
        print(f"  ❌ YYB getCode 失败: {str(data.get('msg'))[:80]}")
    except Exception as e:
        print(f"  ❌ YYB getCode 异常: {str(e)[:80]}")
    return None


def ucmp_app_on_login(code: str) -> Optional[Dict]:
    """wx.login code 换 UCMP 会话"""
    try:
        r = requests.get(f"{UCMP_BASE}/wxaccess/weixin/appOnLogin",
        ...
        print(f"  ❌ appOnLogin 异常: {str(e)[:80]}")
    return None


def get_cookie_from_code(code: str) -> Optional[str]:
    """通过 wx.login code 走 UCMP 换取顺丰 Cookie（sessionId/_login_mobile_/_login_user_id_）"""
    ucmp = ucmp_app_on_login(code)
    if not ucmp:
        print("  ❌ appOnLogin 失败")
        return None
    suuid = ucmp.get("sessionId", "")
    if not suuid:
        print("  ❌ appOnLogin 未返回 sessionId")
        return None

    try:
        s = requests.Session()
        s.verify = False
        ua = ("Mozilla/5.0 (iPhone; CPU iPhone OS 16_2 like Mac OS X) "
              "AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 "
              "MicroMessenger/8.0.69(0x1800452d) NetType/WIFI Language/zh_CN")

        # 查询绑定信息（预热会话）
        try:
            s.post(f"{UCMP_BASE}/wxopen/weixin/wxMemIsBind", json={},
        ...
        sfnew_url = (f"{UCMP_BASE}/wechat-act/weixin/activity/sfnewactivity?"
                     f"bizCode={_q(biz_code)}&regSource=mypoint&citycode=025"
                     f"&cityname={_q('广州')}&wxapp-version=V17.49&suuid={suuid}")
        s.get(sfnew_url, headers={"user-agent": ua,
        ...
        if session_id and (not login_mobile or not login_user_id):
            try:
                s.headers.update({"User-Agent": ua, "Cookie": f"sessionId={session_id}"})
                s.get("https://mcs-mimp-web.sf-express.com/mcs-mimp/app/index.html",
        ...
        if not session_id or not login_mobile or not login_user_id:
            print(f"  ❌ Cookie 不完整 session={bool(session_id)} mobile={bool(login_mobile)} uid={bool(login_user_id)}")
            return None

        parts = [f"sessionId={session_id}", f"_login_mobile_={login_mobile}",
                 f"_login_user_id_={login_user_id}"]
        for k in ["HWWAFSESTIME", "HWWAFSESID", "JSESSIONID"]:
            if cookies.get(k):
                parts.append(f"{k}={cookies[k]}")
        print(f"  ✅ YYB 授权登录成功 ➔ 手机: {login_mobile}")
        return ";".join(parts)
    except Exception as e:
        print(f"  ❌ 换取 Cookie 异常: {str(e)[:80]}")
        return None
...
class MidAutumnExecutor:
    ...
    def _pick_inviter(self) -> str:
        """选择邀请人ID：仅使用自己账号轮转互刷（我的邀请），无固定邀请池"""
...
# ==================== 账号执行 ====================
INVITE_LIST_URL = ('https://mcs-mimp-web.sf-express.com/mcs-mimp/commonPost/'
                   '~memberNonactivity~midAutumn2026TaskService~taskInviteList')


def fetch_invite_used(http) -> Optional[int]:
    """查询账号当前已邀请好友数（用于判断邀请是否满额）"""
    try:
        resp = http.request(INVITE_LIST_URL)
        if resp and resp.get('success'):
            return len(resp.get('obj') or [])
    except Exception:
        pass
    return None


def login_account(account: Dict, index: int) -> Optional[Dict]:
    """登录单个账号（YYB 授权优先，回退 Cookie 格式），返回上下文"""
    logger = Logger(verbose=VERBOSE)
    proxy_manager = ProxyManager(os.getenv('SF_PROXY_API_URL', ''))

    if account['mode'] == 'yyb':
        print(f"\n[{index + 1}] 📱 {account['label']}")
        code = yyb_get_code(account['ref'])
        if not code:
            logger.error(f"{account['label']} YYB 取码失败")
            return None
        cookie = get_cookie_from_code(code)
    else:
        cookie = account['url']
    ...
    try:
        success, user_id, phone = http.login(cookie)
    except Exception as e:
        logger.error(f"登录异常: {str(e)[:80]}")
        return None
    if not success:
        logger.error(f"{account['label']} 登录失败")
        return None
    return {'http': http, 'user_id': user_id, 'phone': phone,
            'index': index, 'label': account['label'], 'inviter_id': ''}


def execute_account(ctx: Dict) -> Dict[str, Any]:
    """执行单账号活动流程"""
    logger = Logger(verbose=VERBOSE)
    http, user_id, phone, index = ctx['http'], ctx['user_id'], ctx['phone'], ctx['index']
    ...
    executor = MidAutumnExecutor(http, logger, user_id, dry_run=DRY_RUN,
                                 inviter_id=ctx.get('inviter_id', ''))
    activity_result = executor.run()
    return {'success': True, 'phone': masked_phone, 'index': index, **activity_result}


# ==================== 主程序 ====================
def main():
    # 账号来源：YYB 授权登录优先；无 YYB 账号时回退 sfsyUrl 环境变量
    accounts: List[Dict] = []
    yyb_list = yyb_get_accounts()
    if YYB_REF:
        refs = [r.strip() for r in YYB_REF.split(',') if r.strip()]
        yyb_list = [a for a in yyb_list
                    if str(a.get("id")) in refs or a.get("openid") in refs]
        if not yyb_list:
            print(f"❌ 未找到指定 YYB ref: {YYB_REF}")
            return
    for a in yyb_list:
        label = (a.get("nickname") or a.get("alias") or f"id={a.get('id')}").strip()
        accounts.append({'mode': 'yyb', 'ref': str(a.get("id")), 'label': label})

    if not accounts:
        env_value = os.getenv('sfsyUrl')
        if env_value:
            print("ℹ️ YYB 无可用账号，回退 sfsyUrl 环境变量")
            raw = [x.strip() for x in re.split(r"@@@|&|[\r\n]+", env_value) if x.strip()]
            accounts = [{'mode': 'ck', 'url': u, 'label': f'CK{i+1}'} for i, u in enumerate(raw)]

    if not accounts:
        print("❌ 未找到任何账号（YYB 无账号且未配置 sfsyUrl）")
        return
    print(f"📱 共获取到 {len(accounts)} 个账号（YYB 授权登录）")

    # 第一阶段：全部登录，收集各账号 user_id
    ctxs: List[Optional[Dict]] = []
    for idx, acc in enumerate(accounts):
        try:
            ctxs.append(login_account(acc, idx))
        except Exception as e:
            print(f"  ❌ 登录异常: {str(e)[:80]}")
            ctxs.append(None)
        if idx < len(accounts) - 1:
            time.sleep(2)

    # 邀请分配（我的邀请）：默认全部使用「爱我中华」的邀请；
    # 该号邀请满额（默认4位，可用 SF_INVITE_MAX 覆盖）后自动切换下一个账号的邀请，以此类推
    invite_max = int(os.getenv('SF_INVITE_MAX', '4'))
    primary = next((x for x in ctxs if x and '爱我中华' in x['label']), None)
    if primary is None:
        primary = next((x for x in ctxs if x), None)
    queue: List[Dict] = []
    if primary:
        queue.append(primary)
        queue.extend([x for x in ctxs if x and x is not primary])

    used_cache: Dict[str, int] = {x['user_id']: -1 for x in queue}   # -1 = 尚未查询实际进度

    def quota_used(cand: Dict) -> int:
        uid = cand['user_id']
        if used_cache[uid] < 0:
            real = fetch_invite_used(cand['http'])
            used_cache[uid] = real if real is not None else 0
        return used_cache[uid]
    ...
    for ctx in ctxs:
        ...
        while p < len(queue):
            cand = queue[p]
            if cand is ctx:
                p += 1          # 不能邀请自己，看下一个（不移动全局指针）
                continue
            if quota_used(cand) >= invite_max:
                if p == pointer:
                    pointer += 1    # 当前主邀请人已满，全局切换到下一个号
                p += 1
                continue
            before = quota_used(cand)
            assigned = cand['user_id']
            used_cache[cand['user_id']] = before + 1
            break
        ctx['inviter_id'] = assigned
        if assigned:
            who = next(x['label'] for x in queue if x['user_id'] == assigned)
            print(f"  {ctx['label']} → 使用邀请: {who}（该号已邀 {used_cache[assigned] - 1}/{invite_max}）")
        elif ctx is primary:
            print(f"  {ctx['label']} → 主邀请人，自身无需被邀请")
    ...
    if CONCURRENT_NUM <= 1:
        for ctx in ctxs:
            if ctx:
                try:
                    all_results.append(execute_account(ctx))
                except Exception as e:
                    print(f"  ❌ 执行异常: {str(e)[:80]}")
                    all_results.append({'success': False, 'phone': ctx['label'], 'index': ctx['index']})
            else:
                all_results.append({'success': False, 'phone': '', 'index': len(all_results)})
    else:
        valid = [c for c in ctxs if c]
        with ThreadPoolExecutor(max_workers=CONCURRENT_NUM) as pool:
            futures = {pool.submit(execute_account, ctx): ctx for ctx in valid}
            for future in as_completed(futures):
                all_results.append(future.result())
        done_idx = {r['index'] for r in all_results}
        for i, ctx in enumerate(ctxs):
            if i not in done_idx:
                all_results.append({'success': False, 'phone': (ctx or {}).get('label', ''),
                                    'index': i})
