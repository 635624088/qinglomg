...
环境变量 (均可选):
  ...
  GH_CONCURRENCY   并发账号数 (默认 -1=全部账号同时并发, 0=退回串行, >0=指定数量)
...
GH_CONCURRENCY = int(env("GH_CONCURRENCY", "-1"))

# 优先级账号(我的账号): 读取 YYB_PRIORITY (openid 换行分隔), 这些账号先串行跑完再跑别人
...
def main() -> int:
    ...
    # 🔥 优先级: 我的账号(YYB_PRIORITY)永远先串行跑完, 再并发跑别人
    if PRIORITY_REFS:
    ...
    # 实际并发数: >0 用指定值; 0=串行; 未设置/负数(-1)=全部账号同时并发
    if GH_CONCURRENCY and GH_CONCURRENCY > 0:
        eff_workers = GH_CONCURRENCY
    else:
        eff_workers = len(accounts)
    log(f"· 并发模式: GH_CONCURRENCY={GH_CONCURRENCY} (实际并发 {eff_workers}/{len(accounts)} 账号)")
    ...
    if mine_idx and GH_CONCURRENCY != 0:
        log(f"· [优先] 我的 {len(mine_idx)} 个账号先串行执行...")
        for i in mine_idx:
            all_results[i] = worker(i, accounts[i])
    ...
    if GH_CONCURRENCY == 0:
        ...
        with ThreadPoolExecutor(max_workers=eff_workers) as pool:
            futures = {pool.submit(worker, i, accounts[i]): i for i in other_idx}
