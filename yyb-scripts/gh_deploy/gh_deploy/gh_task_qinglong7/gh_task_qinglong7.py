...
微信代理服务 (默认 http://172.17.0.1:18080, 提供扫码登录 + 取 code):
  GH_WX_BASE      微信代理服务地址 (如 http://172.17.0.1:18080)
...
class GongHuiClient:
    ...
    def wxlogin(code: str, nickname: str = "") -> tuple[str, str, str]:
        ...
        # 换取 token 后查询一次用户信息，补全 openid / mobile
        try:
...
def normalize_tokens(tokens: dict) -> dict:
    """按 wx_ref 去重归一化, 防止同一账号双写 (oT6 openid / owNAX openid 两套键)。

    规则:
      1. 有 wx_ref 的条目按 wx_ref 分组, 每组只保留一条 (优先 openid+token 齐全者);
      2. 结果 key: openid 非空用 openid, 否则用 wx_ref;
      3. 无 wx_ref 的旧残留条目原样保留。
    """
...
# ============================ 微信代理服务子命令 ============================
def wx_get_token(proxy: WxProxyClient, acc: dict, retries: int = 3) -> Optional[dict]:
    """用代理服务账号换取工惠通 token。

    Args:
        proxy: 代理服务客户端。
        acc: 代理账号 dict (含 id/openid/nickname/status)。
        retries: wxLogin 失败后的重试次数 (默认 3, 含首次)。
    ...
    # 风控/网络/授权失败 → 逐步拉长等待后重试
    if not token:
        for attempt in range(1, retries + 1):
            # 风控类错误用更长退避：10s / 30s / 60s
            if "授权" in msg or "失败" in msg or "500" in msg:
                wait = 10 * (3 ** (attempt - 1))  # 10, 30, 90
            else:
                wait = 5 * attempt  # 5, 10, 15
...
def refresh_token_from_proxy(acc: dict) -> Optional[str]:
    ...
    tokens = load_saved_tokens()
    target = str(acc.get("openid") or acc.get("ref") or "")
    wxr = str(acc.get("wx_ref") or "")
    for key, entry in tokens.items():
        if (str(entry.get("openid") or key) == target) or (
            wxr and str(entry.get("wx_ref") or "") == wxr
        ):
