...
class Task {
  ...
  async run() {
    ...
    // dump full activity config to discover real component actions
    try {
      ...
      const txt = JSON.stringify(cfg.data);
      console.log("PROBE CONFIG_LEN =", txt.length);
      // print anything mentioning prize/record/win/exchange/兑换
      for (const kw of ["兑换","prize","record","win","中奖","code","reward"]) {
        const idx = txt.toLowerCase().indexOf(kw.toLowerCase());
        if (idx >= 0) console.log("PROBE KW", kw, "=>", txt.slice(Math.max(0,idx-80), idx+200));
      }
    } catch (e) { console.log("PROBE CONFIG error", e.message); }

    // do ONE real draw if times>0, dump full raw response
    if (times > 0) {
      const actCsrf = randomHex(16);
      const body = {
...
!(async () => {
  ...
  for (const account of SERVERS.slice(0, 6)) {
    try {
