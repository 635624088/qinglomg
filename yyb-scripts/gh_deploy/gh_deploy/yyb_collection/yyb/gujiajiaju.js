...
class Task {
  ...
  async ensureLogin() {
    ...
    if (cached) {
      ...
      try {
        ...
        $.log(`${this.nickname} 缓存失效，重新登录`);
      }
    }
    await this.login();
    await this.getUserInfo();
    this.saveCachedToken();
    $.log(`${this.nickname} 登录成功 memberId=${this.memberId}`);
  }
  ...
  async checkCalendar() {
    try {
      ...
      $.log(`${this.nickname} 日历查询: code=${ret.code}`);
    } catch (e) {
      $.log(`${this.nickname} 日历查询失败: ${e.message || e}`);
    }
  }
