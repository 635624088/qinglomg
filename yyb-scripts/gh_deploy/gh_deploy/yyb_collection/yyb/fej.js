...
class Task {
    ...
    async loginByWxCode() {
        try {
            ...
            $.log(`${this.nickname} 登录成功: ${data.nick_name || ""} ${maskPhone(data.mobile) || ""}`);
        } catch (e) {
    ...
    async showCheckinPage() {
        try {
            ...
            $.log(`${this.nickname} 签到活动: checkinId=${this.checkinId || "未获取"} isShow=${!!data?.isShow}`);
        } catch (e) {
    ...
    async doCheckin() {
        if (!this.checkinId) {
            $.log(`${this.nickname} 未获取到 checkinId，跳过签到`);
            return;
        }
        try {
            const data = await this.request({
            ...
            $.log(`${this.nickname} 签到成功: ${data?.desc || ""}${awards ? ` ${awards}` : ""}`);
        } catch (e) {
    ...
    async getPoints() {
        try {
            const data = await this.request({ path: "/wscump/integral/user_points.json" });
            $.log(`${this.nickname} 当前积分: ${data?.current_points ?? data?.real_points ?? "未知"}`);
        } catch (e) {
