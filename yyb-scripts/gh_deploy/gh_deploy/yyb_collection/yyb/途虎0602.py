...
def get_code(wxid):
    print(f"\U0001f510 [授权] 请求 YYB 协议获取code: wxid={wxid}")
    try:
...
def main() -> None:
    ...
    if yyb_accounts:
        ...
        for nickname, wxid in yyb_accounts:
            accounts.append({"remark": nickname, "wxid": wxid})
        ...
        if not accounts:
            print("❌ 未找到 YYB 协议和环境变量 tuhu，请设置后重试")
            return
    ...
    send_pushplus("🚗 途虎养车四账号任务完成", build_notify(results))
