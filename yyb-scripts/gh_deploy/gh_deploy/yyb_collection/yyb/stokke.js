...
let wechat = new WeChatServer({ appid: 'wxe232c36aaca3dc1a' });
...
class Task {
    ...
    async getUserInfo() {
        ...
        if (result?.code == '200') {
            //打印签到结果
            $.log(`🌸${this.nickname}` + `[${result.data.nickname}] 积分[${result.data.integral}]🎉`);

        } else {
            $.log(`🌸${this.nickname} 获取用户信息-失败:${result.message}❌`)
        }
    }
    ...
    async doSign() {
        ...
        if (result?.code == '200') {
            ...
            $.log(`🌸${this.nickname} 签到-失败:${result.message}❌`)
        }
