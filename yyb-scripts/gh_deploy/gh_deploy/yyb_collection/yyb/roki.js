...
class Task {
    ...
    async loginByWxCode() {
        try {
            ...
            $.log(`${this.nickname} 登录成功: token=${shortValue(this.token)}`);
        } catch (e) {
    ...
    async getUserInfo(needLog = true) {
        ...
        if (needLog) {
            ...
            $.log(`${this.nickname} 用户: ${name}${mobile} 积分=${points} 今日签到=${this.userInfo.todayIsCheckIn ? "是" : "否"}`);
        }
    ...
    async signIn() {
        try {
            ...
            $.log(`${this.nickname} 签到成功: ${result.message || result.msg || "ok"}`);
            await this.getUserInfo(false);
        } catch (e) {
            $.log(`${this.nickname} 签到失败: ${e.message || e}`);
    ...
    async getRecentSignIn() {
        try {
            ...
            $.log(`${this.nickname} 连续签到: ${days}天`);
        } catch (e) {
            $.log(`${this.nickname} 查询签到记录失败: ${e.message || e}`);
        }
    }
    ...
    async getMemberPoints() {
        try {
            ...
            $.log(`${this.nickname} 当前积分: ${typeof points === "object" ? JSON.stringify(points) : points}`);
        } catch (e) {
