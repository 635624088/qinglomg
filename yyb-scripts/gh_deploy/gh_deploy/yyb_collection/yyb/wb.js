...
class Task {
    ...
    async loginByWxCode() {
        try {
            ...
            this.saveCachedToken();
            $.log(`${this.nickname} 登录成功: userId=${this.userId || "未知"}`);
        } catch (e) {
    ...
    async getUser(silent = false) {
        ...
        this.saveCachedToken();
        if (!silent) $.log(`${this.nickname} 用户: userId=${this.userId || "未知"} 能量${this.currentJf ?? 0}`);
        return user;
    }
    ...
    async signIn() {
        try {
            ...
            $.log(`${this.nickname} 签到记录: 本月${signList.length}天 连续${monthInfo.data?.continuousSignDays || 0}天`);
            ...
            if (callback.code === 200) {
                ...
                $.log(`${this.nickname} 签到成功: +${energy?.rewardValue || justSign.data || "未知"}能量`);
            } else {
                $.log(`${this.nickname} 签到回调失败: ${callback.message || callback.code}`);
            }
        } catch (e) {
        ...
        try {
            ...
            for (const task of tasks) {
                ...
                if (String(task.status) === "2") {
                    $.log(`${this.nickname} ${taskName}: 已完成`);
                    continue;
                }
                ...
                if (String(task.status) === "0") {
                    ...
                    if (ok.code !== 200) {
                        $.log(`${this.nickname} ${taskName}: 标记完成失败 ${ok.message || ok.code}`);
                        continue;
                    }
                }
                ...
                if (complete.code === 200) {
                    $.log(`${this.nickname} ${taskName}: 领取成功 +${task.awardsValue || "未知"}能量`);
                } else if (complete.code === 21210108) {
                    $.log(`${this.nickname} ${taskName}: 奖励已领取`);
                } else {
                    $.log(`${this.nickname} ${taskName}: 领取失败 ${complete.message || complete.code}`);
                }
            }
        } catch (e) {
            const message = e.message || e;
            $.log(`${this.nickname} 任务中心失败: ${message}`);
            if (isTokenError(message)) this.removeCachedToken();
        }
    }
}
