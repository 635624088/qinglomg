...
def validate_proxy(proxies: Dict[str, str] | None) -> Tuple[bool, str]:
    ...
    try:
        ...
        if response.status_code == 200:
            ip = response.json().get("origin", "未知") if response.json() else "未知"
...
def request_with_proxy(method: str, url: str, *, proxies: Dict[str, str] | None = None, server: str = "", **kwargs) -> requests.Response:
...
def get_code(wxid):
    print(f"🔐 [授权] 请求 YYB 协议获取code: wxid={wxid}")
    try:
        response = requests.post(
            f"{YYB_BASE_URL}/wxapp/getCode",
            json={"ref": wxid, "app_id": APPID},
            timeout=20,
        )
        data = response.json()
...
def login_by_code(server: str, code: str, proxies: Dict[str, str] | None) -> Tuple[str | None, Dict[str, Any] | None]:
    try:
        ...
        response = request_with_proxy("POST", LOGIN_URL, headers=common_headers(), json=payload, proxies=proxies, server=server)
        try:
            data = response.json()
        except Exception:
...
def check_sign_status(server: str, token: str, proxies: Dict[str, str] | None) -> Tuple[bool, Dict]:
    ...
    try:
        response = request_with_proxy("POST", SIGN_STATUS_URL, headers=common_headers(token, extra_headers), json=payload, proxies=proxies, server=server)
        data = response.json()
        if data.get("errcode") == 0:
...
def submit_signin(server: str, token: str, proxies: Dict[str, str] | None) -> Tuple[bool, str, int]:
    """提交签到，使用正确的签到接口"""
    extra_headers = {"x-wmsdk-vid": "6016741943359", "x-biz-id": "146", "cloud-project-name": "fansquan",
                      "x-component-is": "onecrm/signgift", "cloud-bosid": "4022115200359", "weimob-bosId": "4022115200359",
    ...
    try:
        response = request_with_proxy("POST", SIGN_SUBMIT_URL, headers=common_headers(token, extra_headers), json=payload, proxies=proxies, server=server)
        data = response.json()
...
def run_account(index: int, total: int, wxid: str, server: str) -> Dict[str, Any]:
    result = {"server": server, "success": False, "proxyStatus": "未使用代理", "proxyIp": "-", "token": "-",
    ...
    try:
        ...
        print("📝 [签到] 检查签到状态...")
        is_signed, sign_data = check_sign_status(server, token, proxies)
        ...
        if is_signed:
            ...
            print("📝 [签到] 未签到，开始签到...")
            sign_ok, sign_msg, earned = submit_signin(server, token, proxies)
...
def main() -> None:
    ...
    if yyb_accounts:
        ...
        accounts = [{"remark": nickname, "wxid": wxid} for nickname, wxid in yyb_accounts]
    else:
        # 回退到环境变量
        raw = os.getenv("tongyiwxid", "")
        lines = [l.strip() for l in raw.split("\n") if l.strip()]
        accounts = []
        for line in lines:
            idx = line.find("#")
            if idx == -1:
                accounts.append({"remark": line, "wxid": line})
            else:
                accounts.append({"remark": line[:idx], "wxid": line[idx+1:]})
        if not accounts:
            print("❌ 未找到环境变量 tongyiwxid，且 YYB 协议无账号")
            return
    ...
    for index, acc in enumerate(accounts, 1):
        try:
            result = run_account(index, len(accounts), acc["wxid"], acc["remark"])
            results.append(result)
        except Exception as exc:
            print(f"❌ [主程序] {acc['remark']} 执行异常: {exc}")
            results.append({"server": acc['remark'], "success": False, "proxyStatus": "-", "proxyIp": "-", "token": "-",
                           "signMsg": "-", "earnedIntegral": "0", "error": traceback.format_exc().strip()})
    ...
    send_pushplus("🏠 优智云家四账号任务完成", build_notify(results))
