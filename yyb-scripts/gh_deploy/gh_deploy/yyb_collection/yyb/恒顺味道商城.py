...
入口: 微信小程序"恒顺味道商城"(有赞零售 wsc)
功能: 每日签到领积分

环境变量:
  YYB_BASE_URL  YYB 服务地址 (默认 http://172.17.0.1:18080)
  YYB_REF       指定账号 ref (可选，默认遍历所有账号)
  YZ_KDT_ID     门店 kdt_id (默认 105297050, 签到接口用)
  YZ_HQ_KDT_ID  总店 kdt_id (默认 105377850, 登录用)
"""
...
# ====== 业务常量 ======
APP_NAME = "恒顺味道商城"
APPID = "wxd78c7ca5dd8f26b2"
KDT_ID = os.getenv("YZ_KDT_ID", "105297050")          # 门店(签到接口)
HQ_KDT_ID = os.getenv("YZ_HQ_KDT_ID", "105377850")    # 总店(登录接口)
UA = ("Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 "
      ...
      "XWEB/20089")
REFERER = f"https://servicewechat.com/{APPID}/105/page-frame.html"
# ======================


def build_extra_data(sid):
    """extra-data 头里的 sid(sessionId) 是识别用户身份的关键，缺它签到接口报错"""
    return json.dumps({
        "is_weapp": 1, "sid": sid,
        "version": "3.197.5", "client": "weapp",
        "bizEnv": "retail",
        "uuid": "gAOqU3ghOr48zSk1784275960871",
...
def login_with_code(code):
    """code 换 accessToken + sessionId"""
    url = (f"https://retail-uic.youzan.com/passport/general/auth.json"
           f"?kdt_id={HQ_KDT_ID}&app_id={APPID}")
    body = {
        "appId": APPID, "code": code, "platformName": "weapp",
        "signature": "windows", "clientBiz": "weapp_wsc",
        "inWsc": True, "kdtId": int(HQ_KDT_ID),
        "extraBizData": {
            "enterOptions": {
                "extKdtId": int(HQ_KDT_ID),
                "path": "pages/home/dashboard/index",
                "query": {}, "scene": 1256,
                "referrerInfo": {}, "apiCategory": "default",
            },
...
def get_checkin_info(sid, token):
    """查 checkInId"""
    url = (f"https://retail-h5.youzan.com/wscump/checkin/"
...
def process_account(account):
    ...
    if act.get("isCheckin"):
        print(f"  [⚠] 今日已签到, 连续{act.get('continuesDay', 0)}天, 跳过")
        return
    print(f"  [→] 今日未签到, 执行签到...")
