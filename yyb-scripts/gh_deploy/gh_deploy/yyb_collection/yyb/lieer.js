...
const TOKEN_CACHE_FILE = path.join(__dirname, "lieer_token_cache.json");
...
class Task {
    ...
    async loginByWxCode() {
        try {
            ...
            this.saveCachedToken();
            $.log(`${this.nickname} 登录成功: userId=${data.userId || ""}`);
        } catch (e) {
    ...
    async getUserInfo(log = true) {
        ...
        if (log) {
            ...
            $.log(`${this.nickname} 用户: ${name}${mobile ? ` ${maskMobile(mobile)}` : ""}`);
        }
        return data;
    }
    ...
    async getPointsInfo() {
        try {
            ...
            $.log(`${this.nickname} 积分: ${data?.pointsNums ?? "未知"} 连签=${data?.seriesDays ?? 0} 今日=${data?.signTodayResult ? "已签" : "未签"}`);
            return data;
        } catch (e) {
            const message = e.message || e;
            $.log(`${this.nickname} 查询积分失败: ${message}`);
    ...
    async getSignList() {
        try {
            ...
            $.log(`${this.nickname} 签到状态: ${this.todayDate} ${this.isTodaySign ? "已签" : "未签"}`);
        } catch (e) {
            const message = e.message || e;
            $.log(`${this.nickname} 查询签到状态失败: ${message}`);
            if (/token|登录|授权|401/i.test(String(message))) this.removeCachedToken();
        }
    }
    ...
    async doSign() {
        ...
        try {
            ...
            $.log(`${this.nickname} 签到成功: +${data?.signReward ?? data?.reward ?? "未知"}积分`);
        } catch (e) {
