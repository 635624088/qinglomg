...
class Task {
  ...
  async run() {
    console.log(`\n账号[${this.index}] ${this.nickname || mask(this.openid || this.cacheKey)}`);
    await this.ensureLogin();
    ...
    const times = await this.lotteryTimes();
    if (times <= 0) {
      console.log(`  └─ 可抽奖次数: 0，本账号暂无可中奖项`);
      return;
    }
    const limit = Math.min(times, Number(process.env.wps_lottery_limit || 5));
    console.log(`  └─ 可抽奖次数: ${times}，本次抽取最多 ${limit} 次`);
    let won = 0;
    for (let i = 1; i <= limit; i++) {
      const actCsrf = randomHex(16);
      const body = {
        component_uniq_number: { activity_number: LOTTERY_ACTIVITY, page_number: LOTTERY_PAGE_NO, component_number: LOTTERY_COMPONENT_NO, component_node_id: LOTTERY_COMPONENT_NODE_ID, filter_params: {} },
        component_type: 45, component_action: "lottery_v2.exec", lottery_v2: { session_id: 1 }
      };
      const res = await wpsRequest("POST", COMPONENT_ACTION, { data: body, cookies: { ...this.cookies, act_csrf_token: actCsrf }, secret: this.secret, headers: { "Content-Type": "application/json", "X-Act-Csrf-Token": actCsrf, Referer: LOTTERY_PAGE } }).catch(e => ({ data: { result: "error", msg: String(e) } }));
      this.cookies = mergeSetCookie([], { ...this.cookies, ...res.cookies });
      this.saveCache();
      const r = res.data;
      if (r?.result === "ok" && r.data?.lottery_v2?.success) {
        const rw = r.data.lottery_v2;
        const isPity = rw.reward_type === "pity" || rw.reward_name === "谢谢参与";
        if (isPity) {
          console.log(`  └─ 抽奖#${i}: 未中奖(谢谢参与)`);
        } else {
          won++;
          const tag = /会员|权益|vip|超级会员/i.test(rw.reward_name || "") ? "[会员权益] " : "";
          console.log(`  └─ 中奖#${i}: ${tag}${rw.reward_name || rw.reward_type} | 类型=${rw.reward_type} | reward_id=${rw.reward_id} | order_id=${rw.order_id}`);
        }
      } else if (r?.result === "error" && r.msg === "risk block") {
        console.log(`  └─ 抽奖被风控拦截(risk block)，本账号停止`);
        break;
      } else {
        const detail = r?.data?.lottery_v2?.error_code ? `错误码${r.data.lottery_v2.error_code}` : (r?.msg || JSON.stringify(r));
        console.log(`  └─ 抽奖#${i}: ${detail}`);
        break;
      }
      await sleep(1000, 1800);
    }
    console.log(`  └─ 本账号本次中奖 ${won} 项`);
  }
}

!(async () => {
  if (!SERVERS.length) return;
  console.log("注：本WPS打卡抽奖奖品为自动到账(会员积分/权益包/会员)，无兑换码字段；order_id为WPS内部订单号，非可兑换CDKey。");
  if (process.env.WPS_PRIZE_POOL !== "0") {
    try {
      const cfg = await axios.get(ACTIVITY_CONFIG, { timeout: 15000, validateStatus: () => true, headers: { "User-Agent": "Mozilla/5.0 MicroMessenger MiniProgramEnv/Windows", Referer: `https://servicewechat.com/${MINI_APP_ID}/249/page-frame.html` } });
      const pool = cfg.data?.value?.prizePool || cfg.data?.data?.value?.prizePool;
      if (Array.isArray(pool)) {
        console.log("=== 本期奖品池 ===");
        pool.forEach(p => console.log(`  [${p.id}] ${p.name}  概率${Number(p.probability) * 100}%`));
      } else {
        console.log("奖品池结构未识别，跳过打印");
      }
    } catch (e) { console.log("奖品池获取失败:", e.message); }
  }
