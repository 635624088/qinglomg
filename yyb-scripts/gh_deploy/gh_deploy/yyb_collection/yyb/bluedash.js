...
class Task {
    ...
    async getUser() {
        ...
        this.user = data || {};
        this.saveCachedToken();
        $.log(`${this.nickname} 用户: ${data?.nickname || ""} ${maskPhone(data?.mobile) || ""} 积分=${data?.score ?? "未知"}`);
        return data;
    }
    ...
    async getSignList() {
        try {
            ...
            $.log(`${this.nickname} 签到记录: 连续${data?.coiledDay || 0}天 今日=${this.isTodaySign ? "已签" : "未签"}`);
        } catch (e) {
    ...
    async doSign() {
        ...
        try {
            ...
            $.log(`${this.nickname} 签到成功: +${data?.score ?? data ?? "未知"}积分`);
        } catch (e) {
