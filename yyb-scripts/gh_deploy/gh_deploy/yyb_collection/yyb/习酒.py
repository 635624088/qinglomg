...
环境变量:
  ...
  OCR_SERVER    滑块验证码识别服务地址（ddddocr），默认 http://192.168.22.214:7777
  GARDEN_SEED_TYPE  播种作物类型，0=自动判断(默认)，1=强制高粱，2=强制小麦
  GARDEN_AUTO_EXCHANGE  自动兑换积分，0=关闭(默认)，1=开启
  GARDEN_AUTO_WINE  自动酿酒，0=关闭，1=开启(默认)
  GARDEN_MONTH_BASE_L  本月酿酒起点升数(默认 0)
...
OCR_SERVER = os.environ.get("OCR_SERVER", "http://192.168.22.214:7777")
...
# 兜底周期 cron（青龙 6 位：秒 分 时 日 月 周）。
# 脚本会把下次执行时间写成"一次性"表达式，一旦那次执行被错过（重启/关机/时间偏差）
# 任务就再也不会触发。这里作为保底调度，确保任务不会永久停摆。
DEFAULT_QL_CRON = os.environ.get("GARDEN_DEFAULT_CRON", "31 8,16 * * *")
# ======================
...
# ====== 缓存模块 ======
_CACHE_FILE = Path(__file__).resolve().parent / "xijiutoken.json"
# 不设固定 TTL，每次运行都验证 token 是否仍有效，过期即重新获取。
_CACHE_TTL = 0

def cache_load(openid):
    ...
    try:
        ...
        return None
    entry = cache.get(openid)
    if not entry:
        return None
    data = entry.get("data")
    # 无 token 字段或 token 无效 → 视为过期
    token = data.get("token") if isinstance(data, dict) else None
    if not token or not _token_valid(token):
        print(f"    [→] 缓存登录态已失效，将重新获取")
        return None
    return data

def cache_save(openid, data, ttl=_CACHE_TTL):
    cache = {}
...
# ====== 验证码 ======
class DdddOcr:
    def __init__(self, server_url="http://192.168.22.214:7777"):
...
class WxAdapter:
    ...
    def _extract_encrypt_key(self, data, _depth=0):
        # 递归深度上限：YYB 返回异常时 inner 可能退化为空 dict 并自我递归，
        # 曾导致 "maximum recursion depth exceeded"，这里硬性截断。
        if _depth > 8:
            return None
        ...
        if isinstance(inner, dict) or isinstance(inner, str):
            parsed = self._extract_encrypt_key(inner, _depth + 1)
            if parsed:
                return parsed
        return None

    def _extract_encrypted_data(self, data, _depth=0):
        # 同 _extract_encrypt_key：限制递归深度，防止异常返回导致爆栈
        if _depth > 8:
            return None
        data = self._decode_jsonish(data)
        if not isinstance(data, dict):
        ...
        for field in ("Data", "data", "result", "rawData"):
            if field in data:
                sub = data[field]
                if not sub or sub is data:
                    continue
                parsed = self._extract_encrypted_data(sub, _depth + 1)
                if parsed:
                    return parsed
        return None
    ...
    def get_user_encrypt_key(self, wxid, appid):
        ...
        # 优先通过 YYB MMTLS 获取
        try:
            body = {"ref": wxid, "app_id": appid, "payload": payload}
            resp = requests.post(f"{YYB_BASE}/wxapp/operateWxData", json=body, timeout=30)
            data = resp.json()
            inner = data.get("result")
            if inner is None:
                d = data.get("data", {})
                inner = d.get("result", d) if isinstance(d, dict) else d
            # 注意：此处原写法为 (inner if isinstance(inner, dict) else data)，
            # 当 inner 非 dict 时会把 data 原样回传造成无限递归。改为带深度上限的兜底。
            parsed = self._extract_encrypt_key(
                inner if isinstance(inner, (dict, str)) else None)
            if not parsed:
                parsed = self._extract_encrypt_key(data, _depth=6)
            if parsed:
                log.info(f"YYB 获取加密密钥成功 version={parsed.get('version')}")
                return {"success": True, **parsed}
        except Exception as e:
            log.warning(f"YYB get_user_encrypt_key 失败，尝试牛子API: {e}")
    ...
    def call_function(self, wxid, appid, data_str):
        ...
        try:
            ...
            if inner is None:
                d = data.get("data", {})
                inner = d.get("result", d) if isinstance(d, dict) else d
            encrypted = self._extract_encrypted_data(
                inner if isinstance(inner, (dict, str)) else None)
            if not encrypted:
                encrypted = self._extract_encrypted_data(data, _depth=6)
            if encrypted:
                return {"success": True, **encrypted}
            return {"success": True, "rawData": inner if isinstance(inner, dict) else data}
        except Exception as e:
            log.warning(f"YYB call_function 失败，尝试牛子API: {e}")
        ...
        if b64_str and isinstance(b64_str, str):
            try:
                ...
                return {"success": True, "signature": res.get("signature"),
    ...
    def get_mobile(self, wxid, appid):
        ...
        return {"success": True, "mobile": item.get("mobile"),
    ...
    def get_session_id(self, wxid, appid):
        ...
        if outer_ok:
            inner = data.get("Data") or data.get("data") or {}
            session_key = (inner.get("session_key") or inner.get("sessionKey") or
...
class GardenClient:
    ...
    def auto_login(self, wxid, server_url=None, appid=APPID, ocr_server=None):
        ...
        # Step 2: 再获取 code → garden authorized_token
...
# ====== 带自动重试的登录 ======
def auto_login_with_retry(client, wxid, wx_server, ocr_server=None, max_retry=None, base_delay=5):
...
def apply_next_cron(dyn_schedule):
    """写入下次执行时间（每日周期 cron，动态计算）。

    ⚠️ 严禁使用一次性表达式（含具体日/月的 6 位 cron）：
    无论是否过期，青龙都会把该行在 crontab.list 里注释掉，任务永久停摆。
    正确做法是把目标时刻写成「分 时 * * *」的每日周期 cron：
    精确度足够（分钟级），且永不过期；每次任务跑完都会按最新成熟时间重写。
    """
    if update_ql_cron_time(dyn_schedule):
        return
    log.warning("⚠️  动态 cron 写入失败，回退为默认周期 cron: %s" % DEFAULT_QL_CRON)
    if not update_ql_cron_time(DEFAULT_QL_CRON):
        log.error("❌ 周期 cron 回退也失败，请手动检查青龙任务的调度时间")
# ======================

# ====== 主函数 ======
def main():
    print("=" * 56)
    print(f"  {APP_NAME} [YYB版]")
    ...
    for i, account in enumerate(accounts, 1):
        ...
        cached = cache_load(openid)

        if cached and _token_valid(cached.get("token", "")):
            print(f"  [→] 使用缓存登录态（36h 有效）")
            client.set_token(cached["token"])
            wx = WxAdapter(WX_SERVER)
            try:
                enc = wx.get_user_encrypt_key(ref, APPID)
                if enc.get("success"):
                    client.set_crypto(enc["encrypt_key"], enc["iv"], version=enc.get("version", 3))
                    print(f"    [✓] 加密密钥已获取  version=%s" % enc.get("version"))
                else:
                    cached = None
            except Exception as e:
                print(f"    [⚠] 获取加密密钥异常: {e}")
        ...
        if not cached or not _token_valid(cached.get("token", "")):
            print(f"  [2/2] 登录 {APP_NAME}...")
            try:
                result = auto_login_with_retry(client, ref, WX_SERVER, OCR_SERVER, base_delay=5)
            except Exception as e:
                print(f"  [✗] 登录异常: {e}，跳过")
                notify_lines.append("👤 %s\n❌ 登录失败: %s" % (display_name, e))
                continue

            print(f"    [✓] 登录结果: token={'✅' if result.get('token') else '❌'}  加密={'✅' if result.get('crypto_ready') else '❌'}")
            if not result.get("token"):
                print(f"  [✗] 登录失败，跳过")
                continue
            cache_save(openid, result)
        ...
        do_daily = cached.get(openid + "_daily") != today if cached else True
        try:
            summary, min_harvest, _brewed = run(client, do_daily=do_daily)
            notify_lines.append("👤 %s\n%s" % (display_name, summary))
            if do_daily:
                c = cache_load(openid) or {}
                c[openid + "_daily"] = today
                cache_save(openid, c)
            if min_harvest is not None:
                all_min_harvests.append((display_name, min_harvest))
            ...
            if TOKEN_INVALID:
                cache_delete(openid)
                print(f"  [⚠] 检测到 token 失效，立即清除缓存并重新登录重试...")
                try:
                    result = auto_login_with_retry(client, ref, WX_SERVER, OCR_SERVER, base_delay=5)
                    if result.get("token"):
                        cache_save(openid, result)
                        today = datetime.now().strftime("%Y-%m-%d"); do_daily = (cache_load(openid) or {}).get(openid + "_daily") != today
                        summary, min_harvest, _brewed = run(client, do_daily=do_daily)
                        notify_lines.append("👤 %s\n%s" % (display_name, summary))
                        if do_daily:
                            c = cache_load(openid) or {}
                        ...
                        print(f"    [✗] 重试登录仍未返回 token")
                        notify_lines.append("👤 %s\n❌ 执行异常: %s" % (display_name, e))
    ...
    if all_min_harvests:
        ...
        for remark, secs in sorted(all_min_harvests, key=lambda x: x[1]):
            ...
            print("   👤 %-12s  %s  （预计 %s 成熟）" % (remark, fmt_remaining_from_seconds(secs), next_time.strftime("%H:%M:%S")))
        next_run_secs = overall_min_secs + 90
        next_run_time = datetime.now() + timedelta(seconds=next_run_secs)
        print("📅 下次执行: %s  （间隔 %s）" % (next_run_time.strftime("%Y-%m-%d %H:%M:%S"), fmt_remaining_from_seconds(next_run_secs)))
        # 动态调度：把「最短剩余成熟时间+90s」写成每日周期 cron（分 时 * * *）。
        # 预留 3 分钟缓冲，避免 crond reload 前时刻已过而被顺延到明天；
        # 若成熟时间在 24h 开外，周期 cron 最早也会在当天该时刻触发一次提前跑（无害，跑完即重写）。
        sched_time = max(next_run_time, datetime.now() + timedelta(seconds=180))
        dyn_cron = "%d %d * * *" % (sched_time.minute, sched_time.hour)
        print("⏰ 下次执行 cron: %s （每日周期·动态，跑完自动重写）" % dyn_cron)
        apply_next_cron(dyn_cron)
        print("NEXT_RUN_SECS=%s" % next_run_secs)
    else:
        default_secs = 1800
        next_run_time = datetime.now() + timedelta(seconds=default_secs)
        print("⚠️  未获取到地块收获时间，使用默认间隔 %s" % fmt_remaining_from_seconds(default_secs))
        print("📅 下次执行: %s" % next_run_time.strftime("%Y-%m-%d %H:%M:%S"))
        sched_time = max(next_run_time, datetime.now() + timedelta(seconds=180))
        dyn_cron = "%d %d * * *" % (sched_time.minute, sched_time.hour)
        print("⏰ 下次执行 cron: %s （每日周期·动态）" % dyn_cron)
        apply_next_cron(dyn_cron)
