...
"""
橘子樱桃（有赞）小程序签到脚本
基于 HAR 文件提取的 AppId、KdtId、CheckinId
"""
...
SCRIPT_NAME = "橘子樱桃有赞签到"

# 从 HAR 文件中提取的关键信息
APP_ID = "wx6a81bb0575a8b55c"
KDT_ID = "122940115"
DEFAULT_CHECKIN_ID = "5347809"
APP_VERSION = "2.224.7.101"                     # HAR 中显示的版本

# 缓存文件默认路径
DEFAULT_CACHE_PATH = str((Path(__file__).resolve().parent / "juziyingtao_cache.json"))

# User‑Agent 保持与 HAR 中一致（Android 微信小程序）
UA = (
...
BASE_HEADERS = {
    ...
    "accept": "*/*",
    "referer": f"https://servicewechat.com/{APP_ID}/49/page-frame.html",
...
class JuziYingtaoSign:
    ...
    def _fetch_access_token(self, code: str) -> tuple[bool, str, str, str]:
        """使用 code 换取 access_token 和 session_id"""
    ...
    def _acquire_session(self, wxid: str, lines: list[str]) -> tuple[bool, str, str, str]:
        """获取或刷新 access_token、session_id、checkin_id"""
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="橘子樱桃有赞小程序签到脚本（wxid登录）")
...
def main() -> int:
    ...
    do_sign = _env_bool("JUZIYINGTAO_DO_SIGN", True) if args.do_sign is None else args.do_sign == "1"
    ...
    if args.wxid.strip():
        ...
        # 回退到环境变量
        if not accounts:
            raw = os.getenv("juziyingtao", "") or os.getenv("JUZIYINGTAO", "")
