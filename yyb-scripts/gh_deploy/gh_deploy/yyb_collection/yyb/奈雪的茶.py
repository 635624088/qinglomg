...
功能：自动获取奈雪点单小程序 code，登录后执行每日签到并查询奈雪币，支持多账号执行，执行结束后统一聚合推送。
   ...
   - 请求体：{"app_id": "wxab7430e6e8b9a4ab", "ref": "openid"}
...
YYB_BASE_URL = (os.getenv("YYB_BASE_URL") or os.getenv("YYB_BASE_URL") or "http://172.17.0.1:18080").rstrip("/")
ACCOUNT_RAW = os.getenv("naixue_openid") or os.getenv("NAIXUE_OPENID") or ""

def fetch_accounts():
    try:
        res = requests.get(f"{YYB_BASE_URL}/accounts", timeout=15)
        data = res.json()
        accs = data.get("data", [])
        print(f"共获取到 {len(accs)} 个账号")
        for i, a in enumerate(accs, 1):
            print(f"   {i}. {a.get('nickname') or a.get('name', '未知')} (openid: {a.get('openid', '')})")
        return [{"openid": a.get("openid", "").strip(), "nickname": a.get("nickname") or a.get("name", "未知")} for a in accs if a.get("openid")]
...
def extract_wx_code(data) -> str:
    ...
    if not isinstance(data, dict):
        raise RuntimeError(f"wxapp/getCode未返回有效JSON：{safe_json(data)}")
    ...
    if isinstance(nested, dict):
        # 兼容 data.data.code 结构
        if isinstance(nested.get("data"), dict):
            value = nested["data"].get("code") or nested["data"].get("wx_code")
            if isinstance(value, str) and value.strip():
                return value.strip()
        # 兼容 data.result.code 结构（wxapp/getCode）
        if isinstance(nested.get("result"), dict):
            value = nested["result"].get("code") or nested["result"].get("wx_code")
            if isinstance(value, str) and value.strip():
                return value.strip()

    raise RuntimeError(f"wxapp/getCode未返回有效code：{safe_json(data)}")

def get_code(openid: str) -> str:
    if not YYB_BASE_URL:
        raise RuntimeError("未配置 YYB_BASE_URL 或 YYB_BASE_URL")

    url = f"{YYB_BASE_URL}/wxapp/getCode"
    print(f"[{mask_account(openid)}] 请求 wxapp/getCode 网关：{url}")
    ...
    print(f"[{mask_account(openid)}] 获取 code 成功")
    return code
...
def run_account(account: dict, index: int, global_proxy: dict | None = None) -> None:
    wxid = account.get("openid", "")
    account_name = account.get("nickname") or mask_account(wxid)
    result = {
        "index": index,
...
def main() -> None:
    ...
    if not accounts:
        print("naixue_openid 未配置，尝试从 bridge 获取...")
    ...
    if not accounts:
        append_notify_result({
            ...
            "ok": False,
            "status": "配置错误",
            "message": "请配置 naixue_openid 或 NAIXUE_OPENID，多个账号用 &、英文逗号、中文逗号或换行分隔",
        })
        return
    ...
    if not YYB_BASE_URL:
        for index, account in enumerate(accounts, 1):
            append_notify_result({
                ...
                "ok": False,
                "status": "配置错误",
                "message": "请配置 YYB_BASE_URL 或 YYB_BASE_URL",
            })
        return
    ...
    for index, account in enumerate(accounts, 1):
        run_account(account, index, global_proxy)
