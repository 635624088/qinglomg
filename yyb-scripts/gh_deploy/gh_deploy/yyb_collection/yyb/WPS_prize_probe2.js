...
class Task {
  ...
  async run() {
    ...
    // 1) dump raw draw response (only if times>0) to discover code fields
    if (times > 0) {
      const actCsrf = randomHex(16);
      const body = {
      ...
      console.log("PROBE RAW DRAW =>", JSON.stringify(res.data).slice(0, 2000));
    } else {
      console.log("PROBE skip draw (times=0)");
    }

    // 2) try record-query actions with page param
    const recordActs = ["lottery_v2.get_prize_record","lottery_v2.my_prize","lottery_v2.winning_record","lottery_v2.get_win_record","lottery_v2.record_list","lottery_v2.prize_record","lottery_v2.query_record","lottery_v2.get_my_prize","lottery_v2.get_record_list","lottery_v2.get_record"];
    for (const act of recordActs) {
      const body = {
        component_uniq_number: { activity_number: LOTTERY_ACTIVITY, page_number: LOTTERY_PAGE_NO, component_number: LOTTERY_COMPONENT_NO, component_node_id: LOTTERY_COMPONENT_NODE_ID, filter_params: {} },
        component_type: 45, component_action: act, lottery_v2: { page: 1, page_size: 50 }
      };
      const res = await wpsRequest("POST", COMPONENT_ACTION, { data: body, cookies: this.cookies, secret: this.secret, headers: { "Content-Type": "application/json", Referer: LOTTERY_PAGE } }).catch(e => ({ data: { result: "error", msg: String(e) } }));
      console.log("PROBE REC", act, "=>", JSON.stringify(res.data).slice(0, 1200));
    }
  }
}
...
!(async () => {
  ...
  for (const account of SERVERS.slice(0, 1)) {
    try {
