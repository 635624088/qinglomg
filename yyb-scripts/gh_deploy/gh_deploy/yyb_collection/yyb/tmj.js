...
class Task {
    ...
    async loginByWxCode() {
        try {
            ...
            this.saveCachedToken();
            $.log(`${this.nickname} 登录成功: ${this.shopName || this.shopId || "未知门店"}`);
        } catch (e) {
    ...
    async ensureIntegralAccount(silent = false) {
        ...
        if (!silent) {
            $.log(`${this.nickname} 积分体系: ${first?.systemName || "未知"} ${first?.integralAlias || ""}`);
        }
        this.saveCachedToken();
    }
    ...
    async findSignActivity() {
        ...
        $.log(`${this.nickname} 签到活动: ${this.signTitle}`);
        this.saveCachedToken();
    }
    ...
    async loadSignInfo() {
        ...
        $.log(`${this.nickname} 签到状态: ${data?.canJoin === false ? "不可参与" : "可参与"} 奖励${integral}积分`);
        this.saveCachedToken();
    }
    ...
    async doSign() {
        try {
            ...
            $.log(`${this.nickname} 签到成功: +${integral}积分`);
        } catch (e) {
