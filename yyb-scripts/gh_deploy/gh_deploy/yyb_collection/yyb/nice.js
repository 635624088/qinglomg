...
class Task {
    ...
    async loginByWxCode() {
        try {
            ...
            $.log(`${this.nickname} 登录成功: ${this.memberId || "未知"} ${maskPhone(this.phone) || ""}`);
        } catch (e) {
    ...
    async getUser() {
        try {
            ...
            this.saveCachedToken();
            $.log(`${this.nickname} 用户: memberId=${this.memberId || "未知"} ${maskPhone(this.phone) || ""}`);
        } catch (e) {
            const message = e.message || e;
            $.log(`${this.nickname} 查询用户失败: ${message}`);
    ...
    async getSignLog(silent = false) {
        ...
        if (!silent) {
            $.log(`${this.nickname} 签到状态: ${this.todaySigned ? "今日已签" : "今日未签"} 连续${showDay}天 积分${totalCoins}`);
        }
        return data;
    }
    ...
    async doSign() {
        ...
        try {
            ...
            $.log(`${this.nickname} 签到成功: +${signData.coin ?? "未知"} 当前积分${signData.total_coins ?? "未知"}`);
            await this.getSignLog(true);
        } catch (e) {
