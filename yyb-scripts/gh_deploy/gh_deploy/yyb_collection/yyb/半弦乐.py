...
SCRIPT_NAME = "半弦月有赞签到"
APP_ID = "wx2cda6c2f33a5db15"
KDT_ID = "153900008"
DEFAULT_CHECKIN_ID = "7177438"
APP_VERSION = "2.234.7"
DEFAULT_CACHE_PATH = str((Path(__file__).resolve().parent / "半弦乐_cache.json"))
...
BASE_HEADERS = {
    ...
    "accept": "*/*",
    "referer": f"https://servicewechat.com/{APP_ID}/8/page-frame.html",
...
class BanxianyueSign:
    ...
    def _fetch_access_token(self, code: str) -> tuple[bool, str, str, str]:
        ...
        for payload in payloads:
            ...
            last = f"登录成功但未返回 accessToken: {_shorten(result)}"
        return False, "", "", last
    ...
    def _acquire_session(self, wxid: str, lines: list[str]) -> tuple[bool, str, str, str]:
        ...
        if entry:
            ...
            if token:
                ...
                lines.append(f"缓存失效，重新获取：{cid_or_msg}")
        ...
        if ok:
            ...
            checkin_id = DEFAULT_CHECKIN_ID
            lines.append(f"{cid_or_msg}，使用默认 checkin_id={checkin_id}")
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="半弦月有赞签到脚本（wxid登录，支持参数缓存）")
    parser.add_argument("--wxid", default="", help="单个 wxid")
    parser.add_argument("--remark", default="", help="配合 --wxid 使用的备注")
    parser.add_argument("--wechat-server", default=os.getenv("YYB_BASE_URL", "http://172.17.0.1:18080"), help="YYB 协议服务地址")
...
def main() -> int:
    ...
    do_sign = _env_bool("BANXIANYUE_DO_SIGN", True) if args.do_sign is None else args.do_sign == "1"
    ...
    if args.wxid.strip():
        ...
        if yyb_accounts:
            ...
            raw = os.getenv("banxianyue", "") or os.getenv("BANXIANYUE", "")
            accounts = parse_accounts(raw)

    if not accounts:
        print("未找到账号")
        print("用法1: python 半弦乐.py --wxid wxid_xxx")
        print("用法2: 设置环境变量 banxianyue='备注#wxid&备注2#wxid2'")
        return 1

    worker = BanxianyueSign(
        yyb_base_url=args.wechat_server.strip() or os.getenv("YYB_BASE_URL", "http://172.17.0.1:18080"),
