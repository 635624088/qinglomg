...
# print(f"✅ 成功读取 {len(SERVERS)} 台内网wxcode服务：")  # 精简：去掉中间过程日志
for item in SERVERS:
    print(f" - {item}")
print("-" * 60 + "\n")
# =================================================================
...
def get_code(server: str) -> str | None:
    ...
    try:
        ...
        if data.get("code") != 0 or not code:
            print(f"[{parsed_server}] 获取code失败")
            return None

        # print(f"[{parsed_server}] 获取code成功")  # 精简：去掉中间登录日志
        return code
    except Exception as exc:
...
def login_by_code(server: str, code: str, proxies: Dict[str, str] | None) -> Tuple[str | None, Dict[str, Any] | None]:
    try:
        # print("🔐 [登录] 使用 code 换 token")  # 精简：去掉中间登录日志
...
def run_account(index: int, total: int, server: str) -> Dict[str, Any]:
    ...
    try:
        ...
        for draw_index in range(1, member_count + 1):
            ...
            # print(f"⏳ [抽奖] 第 {draw_index} 次抽奖前等待 {wait_time}s")  # 精简：去掉等待信息
            sleep(wait_time)
            ...
            if record_id:
                ...
                try:
                    ...
                    pass  # update_lottery_result 接口已下线，静默跳过
