...
class Task {
  ...
  async run() {
    ...
    // try a batch of record action naming variants (component_action)
    const acts = ["lottery.record","record","lottery_v2.my_record","lottery_v2.prize","lottery_v2.query_prize","get_prize_record","my_prize_record","lottery_v2.get_prize_info","lottery_v2.win","lottery_v2.list"];
    for (const act of acts) {
      ...
      console.log("PROBE ACT", act, "=>", JSON.stringify(res.data).slice(0, 400));
    }

    // one real draw, dump full raw
    if (times > 0) {
      const actCsrf = randomHex(16);
      const body = {
        component_uniq_number: { activity_number: LOTTERY_ACTIVITY, page_number: LOTTERY_PAGE_NO, component_number: LOTTERY_COMPONENT_NO, component_node_id: LOTTERY_COMPONENT_NODE_ID, filter_params: {} },
...
!(async () => {
  ...
  const idx = Number(process.env.PROBE_IDX || 24); for (const account of SERVERS.slice(idx, idx+1)) {
    try {
