...
class MiniDxConstId {
    constructor(openid, nickname) {
        ...
        this.options = { ...DX_MINI_CONFIG, ...openid };
...
class Task {
    ...
    getCachedToken() {
        const item = readCache()[this.openid];
    ...
    async loginByWxCode() {
        ...
        if (!this.token) { $.log(`${this.nickname} 登录响应未返回 token`); return; }
        this.saveCachedToken();
        $.log(`${this.nickname} 登录成功: token=${shortValue(this.token)} lmid=${shortValue(this.lmid)}`);
    }
    ...
    async signIn() {
        ...
        $.log(`${this.nickname} 活动: ${pageInfo.task_name || "签到"} 今日=${this.todaySigned(pageInfo) ? "已签到" : "未签到"}`);
        if (this.todaySigned(pageInfo)) return;

        const dxToken = await getDxToken();
        $.log(`${this.nickname} 风控指纹${dxToken ? "获取成功" : "获取失败，直接尝试"}`);
        ...
        $.log(`${this.nickname} 签到成功${this.rewardText(result?.data?.reward_info) ? `: ${this.rewardText(result.data.reward_info)}` : ""}`);
    }
    ...
    async run() {
        ...
        if (cached) {
            ...
            $.log(`${this.nickname} 使用缓存token: ${shortValue(this.token)}`);
        ...
        try {
            ...
            $.log(`${this.nickname} 签到失败${e.code ? `(${e.code})` : ""}: ${e.message || e}`);
